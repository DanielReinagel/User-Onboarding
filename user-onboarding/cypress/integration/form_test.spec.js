describe('Form App Tests', ()=>{
    ...
    const checkErrors = (nameErr, emailErr, pwErr, toSErr) => {
        if(nameErr){
            cy.contains(nameErr).should('exist');
        } else {
            cy.contains(noName).should('not.exist');
            cy.contains(shortName).should('not.exist');
        }
        if(emailErr){
            cy.contains(emailErr).should('exist');
        } else {
            cy.contains(noEmail).should('not.exist');
            cy.contains(invalidEmail).should('not.exist');
        }
        if(pwErr){
            cy.contains(pwErr).should('exist');
        } else {
            cy.contains(noPw).should('not.exist');
            cy.contains(shortPw).should('not.exist');
        }
        if(toSErr){
            cy.contains(toSErr).should('exist');
        } else {
            cy.contains(toSNotChecked).should('not.exist');
        }
        if(nameErr||emailErr||pwErr||toSErr){
